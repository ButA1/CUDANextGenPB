...
def collect_folder(folder, csv_path, sweep, metric_col, accuracy=None):
    ...
    with open(csv_path, newline="") as fh:
        ok = [r for r in csv.DictReader(fh) if r.get("status") == "ok"]
    # The `folder` column holds the absolute path of the machine that ran the
    # sweep, so a CSV copied back from the cluster records /beegfs/... and never
    # matches the local path. That column only has to disambiguate when one CSV
    # holds several folders (the --csv case); a single-folder file is this
    # folder's by construction, whatever path it was written on.
    multi = len({r.get("folder") for r in ok}) > 1
    rows = [r for r in ok if r.get("sweep") == sweep
            and (not multi or (r.get("folder") or want) == want)]
    if not rows:
        return None
...
def build_caption_notes(datasets, default_machine, overrides, extra_fmm,
    ...
    for ds in datasets:
        ...
        if mol in overrides:
            ...
            # The grid scale used to be stated here too. It is a mesh parameter,
            # already carried per test case by the setup section's test-case
            # table, and it does not change what the three bars are being
            # compared on -- they share a discretisation whatever its scale.
...
ENERGY_MACROS = ["energyMolecules", "energyRepeats", "energyMachine",
                 "energyFmmConfig", "energyReference", "energyCaveats"]


def localise(text, tag):
    r"""Suffix the caption macros and resolve REFLABEL, so two of these figures
    can coexist in one document.

    Without it every --tag emits the same \label and the same \def, and the
    ...
    identity on the labels: "energy_methods" -> "energy-methods".
    """
...
def write_ref_tex(path, datasets, nrep, machine, fmm_note, caveats, refs):
    ...
    body = [
        ...
        "\\def\\energyMolecules{%s}\n" %
        ", ".join(ds["molecule"] for ds in datasets),
        "\\def\\energyRepeats{%s}\n" % nrep,
        "\\def\\energyMachine{%s}\n" % machine,
        "\\def\\energyFmmConfig{%s}\n" % fmm_note,
        "\\def\\energyReference{%s}\n" % refs,
        "\\def\\energyCaveats{%s}\n" % caveats,
    ]
    with open(path, "w") as fh:
        fh.write(localise("".join(body), tag))
...
# The legend has room for the complexity, an 8-column table at 12pt does not.
TABLE_LABELS = {"cpu": "CPU", "naive": "naive GPU", "fmm": "FMM"}


def fmt_spread(m):
    """Half the fastest-to-slowest range, relative to the median.

    The absolute "+3.74/-1.75" form is the widest thing in the table by a wide
    margin and it scales with the value, so the H1N1 and 1CCM rows cannot share
    a column width with it. One relative number says the same thing about
    run-to-run stability and is the same width at every magnitude."""
    if m["n"] < 2 or not m["med"]:
        return "--"
    return "$\\pm${:.1f}\\%".format(100.0 * (m["max"] - m["min"]) / 2.0 / m["med"])
...
def write_table(path, datasets, dnf_limit_s):
    ...
    lines = [
        "% generated by scripts/plot_energy_methods.py -- do not edit",
        # The caption macros live in the ref file. Pulling them in here as well
        # as in the figure costs nothing (it is a handful of \def) and means the
        # table can be \input on its own, or before the figure, without
        # collapsing into a pile of undefined control sequences.
        "\\input{figures/data/%s_ref}" % os.path.basename(path)[:-len("_table.tex")],
        "\\begin{table}[!htb]",
        "\\centering",
        "\\footnotesize",
        # Nine columns are 18 gutters, and \tabcolsep does not scale with the
        # font -- at the default 6pt that is 108pt of pure whitespace, which put
        # the tabular 10.85pt over a 455pt textwidth. 5pt buys back 36pt and is
        # the cheapest fix that keeps every column; dropping one would lose data
        ...
        "molecule & method & ranks & stage [s] & spread & vs.\\ CPU "
        "& vs.\\ naive & polarization & ionic \\\\",
        "\\midrule",
    ]
    for di, ds in enumerate(datasets):
        ...
        for key, _, _, _, _ in METHODS:
            ...
            if m["dnf"]:
                floor = m.get("floor")
                stage = "$>%s$" % hours(floor) if floor else "DNF"
                lines.append(
                    "{} & {} & {} & {} & DNF & -- & -- & -- & -- \\\\".format(
            ...
            lines.append(
                ...
                "{} & {} & {} & {:.4g} & {} & {} & {} & {} & {} \\\\".format(
                    mol, labels[key], m["ranks"] or "--", m["med"],
                    fmt_spread(m),
    ...
    with open(path, "w") as fh:
        fh.write(localise("\n".join(lines),
                          os.path.basename(path)[:-len("_table.tex")]))
