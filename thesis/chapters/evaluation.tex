...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        VRAM & 10GB & 40GB & 80GB \\
        FP32:FP64 & 64:1 & \multicolumn{2}{c}{2:1} \\
...
To my surprise, \texttt{loc\_\allowbreak{}refinement} yielded worse results in my testing, and served only to test \texttt{energy\_\allowbreak{}cuda}.
...
Aside from the additional compiler flags added due to inclusion of CUDA, I largely used the same flags from the original makefile and \texttt{local\_\allowbreak{}settings}. 
...
I reverted to \texttt{-O2}, but purposefully stayed away from the commented-out \texttt{-Ofast -mtune=native -march=native} setting lingering inside the \texttt{local\_\allowbreak{}settings}. While it would have provided even faster baseline performance, \texttt{-Ofast} in particular implies \texttt{-ffast-math}, which permits reassociation of floating point sums, assumes finite arithmetic, and on x86 links \texttt{crtfastmath.o} that enables flush-to-zero and denormals-are-zero process-wide, thereby affecting the linked dependencies as well.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.9cm}p{2.8cm}p{2.0cm}p{2.8cm}p{4.7cm}}
        ...
        \midrule
        test0 & 1CCM.pqr & Comparative & \texttt{mesh\_\allowbreak{}shape}=0, \texttt{perfil1}=0.95,\newline\texttt{perfil2}=0.2, \texttt{scale}=2.0 &
        \multirow{6}{4.7cm}{\texttt{bc\_\allowbreak{}type}=1\newline$\varepsilon_m$=2, $\varepsilon_s$=80\newline\texttt{ionic\_\allowbreak{}strength}=0.145\,mol/L\newline$T$=298.15\,K\newline\texttt{calc\_\allowbreak{}energy}=2\newline\texttt{calc\_\allowbreak{}coulombic}=1} \\
        test1 & 6VYB.pqr & Comparative & \texttt{mesh\_\allowbreak{}shape}=0, \texttt{perfil1}=0.95,\newline\texttt{perfil2}=0.2, \texttt{scale}=2.0 & \\
        test2 & 1vsz.pqr & Comparative & \texttt{mesh\_\allowbreak{}shape}=0, \texttt{perfil1}=0.95,\newline\texttt{perfil2}=0.2, \texttt{scale}=2.0 & \\
        test3 & UNITSPHERE\_-\newline DISC\_2.pqr & Analytical & \texttt{mesh\_\allowbreak{}shape}=0, \texttt{perfil1}=0.15,\newline\texttt{perfil2}=0.15, \texttt{scale}=2.0 & \\
        test4 & 30sfere.pqr & Analytical & \texttt{mesh\_\allowbreak{}shape}=0, \texttt{perfil1}=0.20,\newline\texttt{perfil2}=0.20, \texttt{scale}=2.0 & \\
        test5 & 6VYB.pqr (cluster) & Comparative & \texttt{mesh\_\allowbreak{}shape}=0, \texttt{perfil1}=0.95,\newline\texttt{perfil2}=0.2, \texttt{scale}=2.0 & \\
        \bottomrule
    \end{tabular}
...
Unfortunately, I could only use \texttt{ncu} locally since it requires \texttt{NVreg\_\allowbreak{}RestrictProfilingToAdminUsers} to be disabled.
