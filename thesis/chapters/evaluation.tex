...
As already mentioned in \ref{sec:implementation-energy-calculation}, FP64 performance on gaming/consumer such as the RTX 3080 is neutered when compared to the A100, despite both chips being part of one architecture generation. 
An FP64-intensive kernel running on the RTX 3080 might obscure other performance bottlenecks.
...
For more exact timings in the GPU specific optimizations, I profiled the kernels using Nvidia Nsight Compute (NCU), which runs them multiple times, and eliminating variances. 
...
Unfortunately, I could only use NCU locally. Combined with the limited FP64 performance I was not able to get the full picture needed to optimize the CUDA code in a way that would be reflected on the cluster.
...
I avoided profiling the linear solvers running on the GPU, because I made no code changes to them, by selecting the built in solvers like LIS instead. This way I could avoid putting 
...
One of the goals of this thesis was to maintain NGPB's level of accuracy measured in relative error magnitude of the various energies, as seen in table \ref{tab:ngpb-error-magnitudes}. 
The analytical reference values were graciously provided to me by the authors of NGPB, as the ones in the paper are truncated too early to be useful. Those can be seen in table \ref{tab:ngpb-analytical-values}.
...
All modifications I made combined must not increase those magnitudes for these two specific cases, meaning the magnitude of their error contributions must be on the same level or lower. 
On the other hand, error magnitudes smaller or equal $e^{-12}$ are considered FP64 machine level noise, so aiming for those is pretty much pointless, giving me a wiggle room between $e^{-10}$ and $e^{-12}$ for possible approximations. 
...
RAM utilization wasn't the main focus during testing, as most of it was caused by BIMPP, which I profiled using Heaptrack. 
...
Since rank 0 is a sort of a master rank, which performs important coordination work, it can be easily identified as it will use more memory than the other ranks.
