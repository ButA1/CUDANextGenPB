...
On the other hand, the Intel Xeon E5-2630v4 of the cluster is quite old, and is handily beaten by the R7 5800X3D in both single-core and multi-core performance, which makes comparing performance a little awkward.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        CPU & AMD R7 5800X3D & \multicolumn{2}{c}{Intel Xeon E5-2630v4} \\
...
% for now I am leaving this here unless I need this explanation in the implementation
...
Resolution with this mesh type is controlled through \texttt{scale}. Its value is equal to the inverse resolution in Ångström, meaning that a value of 2 corresponds to a resolution of 0.5 \,\AA{}.
% not sure if I should add SI units. I won't have a lot of units in this thesis
\texttt{mesh\_shape = 1} is a uniform cubic mesh, whose resolution isn't set by \texttt{scale}, but rather by a different parameter called \texttt{unilevel}, and works very differently from \texttt{scale}. 
This led to some initial confusion, and a transition to using \texttt{mesh\_shape = 0} exclusively. 
...
As already explained in \ref{sec:background-sparse-matrices-distributed}, the total memory consumption of all nodes increases slightly with the number of ranks, as each one needs to keep track of ghost- and non-local elements.
...
So I reverted to \texttt{-O2}, but purposefully stayed away from the commented-out \texttt{-Ofast -mtune=native -march=native} setting lingering inside the \texttt{local\_settings}. While it would have provided even faster baseline performance, \texttt{-Ofast} in particular implies \texttt{-ffast-math}, which permits reassociation of floating point sums, assumes finite arithmetic, and on x86 links \texttt{crtfastmath.o} that enables flush-to-zero and denormals-are-zero process-wide, thereby affecting the linked dependencies as well.
...
Instead it's the result produced by the version containing the triangulation fix described in \ref{sec:nextgenbp-submodules-energy-calculation}, as it is more accurate than the result produced by the original code.
...
Since BIMPP was also partially optimized, I needed to switch between the unmodified and modified versions.
...
Unfortunately, I could only use \texttt{ncu} locally since it requires \texttt{NVreg\_RestrictProfilingToAdminUsers} to be disabled.
...
On the other hand, error magnitudes smaller than or equal to $10^{-12}$ are considered FP64 machine-level noise, so aiming for those is not necessary, giving me some wiggle room between $10^{-10}$ and $10^{-12}$ for possible approximations. 
