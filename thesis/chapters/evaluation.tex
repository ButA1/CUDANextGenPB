...
I've plotted their speed against their relative error compared to the CPU reference, taken from an average of 3 runs,
...
Since it was my goal to achieve a relative error of the polarization energy in the order of $10^{-10}$ or lower, two configurations for test1 seem to be hitting that goal while being the fastest: $P = 9$, $\theta = 0.3$, $n_{leaf} = 64$ and $P = 10$, $\theta = 0.4$, $n_{leaf} = 256$.
...
I demonstrated this effect for 6VYB in the left diagram of the Figure~\ref{fig:leaf_optimum_shift_6vyb}, where the optimal $n_{leaf,src}$ rises monotonically with the order of $P$.
...
On top of it all, the number of ranks also influence the balance, though in a predictable way.
...
Even though regions are not equally populated, this still roughly equates to the number of particles being divided by the number of ranks.
...
Note that despite having the same $n_{leaf,src}$ and $n_{leaf,tgt}$, their radically different $P$
...
%   - Fig. leaf_optimum_shift: the minimum walks monotonically toward larger
%     source leaves as P rises, because P enters the cost of an M2L but NOT the
