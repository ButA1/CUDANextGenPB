...
It also led to the discovery that, due to NGPB evaluating the molecular atoms against the surface, having a shared leaf size for both trees was actually detrimental.
...
This has nothing to do with NGPB's convention of suffixing functions, which assume an unrefined grid, with \snakecase{_fast}.
...
This shows that despite using two separate loops, computing the ray casts and distributing them to the other ranks is much quicker than doing it on every loop.
...
I forked the official BIMPP repository to do my implementations, and built and installed it on my local system.
...
The move from \snakecase{std::map} to \snakecase{boost::container::flat\_map} led to a 2.14x reduction of the host memory usage during \snakecase{assemble\_system\_matrix}, which was better than the predicted 2x reduction.
...
The totals are Heaptrack's peak heap memory consumption over the whole run, while the numbers for \snakecase{assemble\_system\_matrix} are the sum of its allocations.
This change had no impact on accuracy; however, it had a slightly positive impact on the stage time of \snakecase{assemble\_system\_matrix}, going from roughly 22 seconds to 20.
...
\input{figures/bimpp_memory_fix}

\subsubsection{bim3a\_solution\_with\_ghosts}

My optimized \snakecase{bim3a_solution_with_ghosts} reduced the time taken for 6VYB from 10 seconds to just 60 milliseconds, which is an extreme speedup of almost 170x.
This moves it from being a significant contributor of the overall time to basically negligible.
Since it is used at the end of every solver, and twice inside \snakecase{create_density_map}, this removed roughly 30 seconds from the overall total time for the full NGPB pipeline, with \snakecase{create_density_map} alone shortening from 24 seconds to just 2.5 seconds.
...
Some relative errors are slightly lower for me, while others are slightly higher.
The order of magnitude stayed the same across the board.
The differences are shown in Table~\ref{tab:baseline-accuracy}.
Interestingly, the triangulation-fix had no impact on these tests.
The relative errors were exactly identical.

Although AMGX was set to a lower residual, it did not increase accuracy.
In fact, it worsened the polarization error for the Kirkwood sphere test slightly when compared to the LIS result, which is still better than the result from the paper.
AMGX did genuinely reach a lower residual (1.195268e-09) than LIS (5.921088e-07), suggesting this is not a limitation of the software, but rather a limitation of the mathematical method.
...
Figure~\ref{fig:solver-bench} compares the total stage time of the LIS and AMGX paths for 1CCM, 6VYB and 1VSZ running on the local system.
...
As LIS runs entirely on the CPU, its entire time is attributed to the host.
This shows that the CPU time for the AMGX path, made up of the data gathering and transfer, is significant.

\input{figures/solver_bench}
\input{figures/solver_bench_table}
...
Upon inspecting the number of quadrants and nodes for each molecule, 6VYB's grid had 28008707 nodes and 28294120 quadrants, while 1VSZ had 15879325 nodes and 16073576 quadrants.
The number of nodes equates to the dimension of the solution vector of the linear equation, which explains why 1VSZ's solve time is quicker.
...
\input{figures/molecule_shapes}

The number of iterations is shown on top of the columns, as well as in Table~\ref{tab:solver-bench}.
Even though the default ``CG-Jacobi-Block'' consistently needs a lot more iterations than both LIS and the tuned configuration (in cases of 6VYB and 1VSZ 10x as many as the tuned configuration), it was still much quicker than LIS, and only marginally slower than the tuned configuration.
This demonstrates that having very cheap iterations can compensate for their number.
...
Surprisingly the ``slower'' A100-node's CPU managed to run LIS faster than the local CPU, probably due to running with 16 ranks instead of 4.
...
I unfortunately do not have enough data to determine whether this is due to my rank-gathering or to something else.
Nevertheless this would have warranted a follow-up investigation.

\input{figures/solver_bench_cluster}
\input{figures/solver_bench_cluster_table}

\subsubsection{Scaling Across Topologies}

I then compared the AMGX solve time for 6VYB for four different topologies: the local system, one cluster node with a single A100 GPU, one with dual A100 GPUs and two nodes with one A100 each, shown in Figure~\ref{fig:amgx_scaling_6vyb}.
...
Increasing the number of ranks to 16 reduces the host time, disproving that a higher rank count makes the host time more expensive.
Instead it suggests that a significant contributor to the host time might not be the gathering after all.

Figure~\ref{fig:amgx_scaling_6vyb} also shows that AMGX scales much better across multiple GPUs per node, but not on multiple nodes.
The tuned configuration performed significantly worse on two nodes than it did on a single node.
This is very likely due to communication overhead, which is much lower on a single node with multiple GPUs, as the latency measurements in Table~\ref{tab:mpi-latency-table} show.
On the dual-GPU node the tuned AMGX performed much better than on the single-GPU node.
Surprisingly, the default configuration did not suffer as much, although it was barely any faster than on a single-GPU node.
...
I made four sweeps of $P$ and $\theta$ for values from 6 to 10 and 0.2 to 0.8 for 6VYB and 1VSZ on my local machine.
...
I have plotted their speed against their relative error compared to the naive $\mathcal{O}(N^2)$ GPU sum over the same inputs, taken from an average of 3 runs, which can be seen in Figures~\ref{fig:fmm_presplit_6vyb} and \ref{fig:fmm_presplit_1vsz}.
...
Gray marks the Pareto front over the whole sweep, and the dashed vertical line the naive path's own time.
...
\input{figures/fmm_presplit_6vyb}
\input{figures/fmm_presplit_1vsz}

\subsubsection{Splitting the Leaf Size}

Since it was my goal to achieve a relative error of the polarization energy equal to or lower than the Kirkwood sphere result in \ref{evaluation-results-baseline}, I chose the two fastest configurations that reached this goal: $P = 9$, $\theta = 0.3$, $n_{leaf} = 64$ and $P = 10$, $\theta = 0.4$, $n_{leaf} = 256$.
Profiling both configurations using \snakecase{ncu} revealed that the two configurations are quick for very different reasons.
...
At a grid scale of 2, the surface points far outnumber the atoms in the molecule, and they are also much denser.
Clustering both trees with the same leaf count causes wildly different geometric leaf sizes.
M2L cost tracks with the total number of box pairs computed by the DTT, which was dominated by the number of box nodes of the much denser target tree.
...
That the two best settings sat at an $n_{leaf}$ of 64 and 256, but not 128, showed that this relationship was an inverted U-shaped curve, with no optimum in between.
...
Setting them to 64 and 256 respectively at $P = 10$, $\theta = 0.4$ cut the time taken by FMM in half.

\subsubsection{Leaf-Size Optima}

Next I tried to find the optima for both values.
As seen in Figures~\ref{fig:leaf_sweep_6vyb} and \ref{fig:leaf_sweep_1vsz} the optimal values of $n_{leaf,src}$ and $n_{leaf,tgt}$ diverged in different directions.
It also shows that they differ between the two molecules, which is probably due to their different shapes, leading to different particle distributions.

\input{figures/leaf_sweep_6vyb}
\input{figures/leaf_sweep_1vsz}

I initially assumed that the ideal leaf counts would be exactly where the geometric leaf sizes of both trees would be equal, but further testing demonstrated that the optima shifted with different $P$ and $\theta$.
This can be explained by the cost of P2P and M2L not being equal.
The balance shifts toward P2P when the cost of M2L increases through raising $P$.
I demonstrated this effect for 6VYB in the left diagram of Figure~\ref{fig:leaf_optimum_shift_6vyb}, where the optimal $n_{leaf,src}$ rises monotonically with the order of $P$ (4, 4, 8, 8, 16, 16 for $P$ 6..11).
Meanwhile $\theta$ has a much weaker secondary effect, flattening the curve as it increases but not relocating the optimum.
Over $\theta = 0.3$ to $0.6$ the optimum does not move at all.

\input{figures/leaf_optimum_shift_6vyb}
\input{figures/leaf_optimum_shift_6vyb_table}

\subsubsection{Platform and Rank Shift}

This also means that this balance is shifted once again on the cluster, where the cost of P2P is greatly reduced, thanks to higher FP64 performance.
The left diagrams of Figures~\ref{fig:platform_shift_1vsz} and \ref{fig:platform_shift_6vyb} both show this effect through an increase in optimal $n_{leaf,src}$, which increases the number of P2P calculations through the two mechanisms explained above.
On top of it all, the number of ranks also influences the balance.
Each rank holds an exact copy of the source tree, but its target tree is built only from the particles of the domain region owned by the rank.
...
\input{figures/platform_shift_6vyb}
\input{figures/platform_shift_6vyb_table}

Though the same cannot be objectively said about 1VSZ, it is clear from the curve that the A100 16-rank curve is almost flat between 512 and 1024, while the A100 4-rank curve has much clearer minima.

In terms of accuracy: the rank 4 results from the A100 and the RTX 3080 are within a $10^{-12}$ relative error.
An increase in ranks tended to decrease the median error slightly, but this was only observed across 62\%-67\% of all runs.
I will refrain from calling this a general trend; I would rather attribute it to fluctuations, in line with the observations made in \ref{sec:evaluation-experimental-setup-mpi-ranks}.

\input{figures/platform_shift_1vsz}
\input{figures/platform_shift_1vsz_table}
...
I converged toward a local optimum through successive sweeps of the best configuration.

One discovery I made while searching was that accuracy did not always monotonically increase with $P$.
As an example: for 1VSZ and $\theta = 0.3$, $n_{leaf,src} = 16$, $n_{leaf,tgt} = 512$ the relative error increased from 4.1e-11 to 6.8e-10, going from $P = 7$ to $P = 8$.
This necessitated the sweep across all $P$ every time I converged on a new optimum.
Nevertheless it is not unexpected.
Cheng, Greengard and Rokhlin give an upper bound on a multipole converted to a local expansion with Theorem 2.4 of \cite{cheng1999fastadaptive}, and it roughly decays with $(\text{separation ratio})^{(P+1)}$ scaled by the sum of the absolute charges.
However, three things made me decide against striving toward it:
First, the atoms are carrying both negative and positive charges, meaning their net charge may be much lower than the sum of their absolutes.
...
Second, this bound makes no judgment on speed, just accuracy, and Salmon \& Warren have also shown that a few percent of accuracy may cost about 30 times as much in computation speed.
Lastly, it does not mean that the error for a single result will be exactly on that bound, or that it will also decrease monotonically with $P$.
...
I was unable to find a configuration for 1CCM that beat naive, so I settled for the best one from a sweep of 140 configurations, which was $P=6$, $\theta=0.5$, $n_{leaf} = 256$.
...
\input{figures/fmm_ptheta_6vyb}
\input{figures/fmm_ptheta_6vyb_table}
\input{figures/fmm_ptheta_1vsz}
\input{figures/fmm_ptheta_1vsz_table}
\FloatBarrier
...
Aside from 1CCM, FMM managed to bring a 4.7x-7.5x speedup over the naive kernel, let alone the CPU path, compared to which it managed a speedup of 117x-127x.
The naive kernel also brought a massive speedup over the CPU path, ranging from 7.7x for 1CCM to 24.6x for 1VSZ.
...
\input{figures/energy_methods}
\input{figures/energy_methods_table}
...
The LIS and naive kernel figures for H1N1 were recorded on a single node with 4 H200 GPUs, while the FMM was run on two H200 nodes, each with two GPUs.
...
Now running on a workstation GPU, the naive kernels are not FP64 bound anymore, resulting in ludicrous gains of 286x and 321x for 6VYB and 1VSZ respectively, as seen in Table~\ref{tab:energy-methods-cluster}.
This really showcases how well GPUs can boost performance through brute force.
...
I have to point out that I did not manage to capture the CPU time for H1N1, which ran out of time within its 12-hour slot, of which the other stages only used 49 minutes.
Even a conservative estimate of 11 hours would mean the naive kernels --- having taken 52 minutes --- are at least 12.5x faster.
Given that the speedup of the naive kernel for 1VSZ and 6VYB on the A100 node was roughly around 300x, it is plausible that the CPU implementation would have taken multiple weeks to finish.
...
Due to the lack of a CPU reference, I had to compare the accuracy of FMM with the naive kernel.
Given that the naive kernel is the exact math, not an approximation, this should be a perfectly fine comparison, which can be seen in Table~\ref{tab:energy-methods-cluster}.
...
Note that I am comparing the best local FMM parameter configuration for 6VYB, while the configuration for the cluster has a $n_{leaf,src}$ of 32 instead of 16, which performs better on the A100.
Unlike AMGX, neither FMM nor the naive kernels need to communicate during the computation.
Therefore the two-node setup scales normally.
In the case of FMM, it scales even better than the single dual-GPU node.
...
The settings for the CUDA-enabled steps were picked from the best configurations of the previous results for the local system.
...
This result shows that the more complex the molecule, the greater the payoff becomes.
...
\input{figures/total_time}
\input{figures/total_time_table}
\FloatBarrier
