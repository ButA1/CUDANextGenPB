...
Analogous to the linear solver comparison, I ran 6VYB, 1VSZ, as well as H1N1 on the cluster, which can be seen in Figure~\ref{fig:energy-methods-cluster} and Table~\ref{tab:energy-methods-cluster}.
...
Now running on a workstation GPU, the naive kernels are not FP64 bound anymore, and the gap between them and FMM, which existed on the local machine, basically disappears for 1VSZ and 6VYB.
1VSZ is now slightly faster using the naive kernels, while 6VYB is slightly faster using FMM.

Nevertheless, the gap reappears on H1N1, where FMM once again shows a massive speedup over the naive kernel, despite it using an old pre-split leaf-count configuration of $P = 10, \theta = 0.4, n_{leaf} 256$.
The three results demonstrate again how FMM pays an upfront cost, that gets pays off massively once the problem becomes compute-bound.
...
Given that the speedup of the naive kernel for 1VSZ and 6YVB on the node A100 was 321x and 286x, I could have been that the CPU implementation would have taken multiple weeks to finish.
...
%
%  Unlike the other figures in this chapter this one does not come from a
%  bench_sweep.py bench_runs.csv -- these runs were made after those sweeps and
%  the raw per-run times live in figures/data/total_time_runs.csv, which is the
%  script's input. Edit that file, not the generated .dat/.tex.
% ------------------------------------------------------------------------
