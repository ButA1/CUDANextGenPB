...
%
%  Heaptrack, 6VYB, rank 1, both captures on the LIS solver (verified by
%  filtering each profile for lis_/amgx_compute_electric_potential -- the
%  earlier "AMGX" label was carried over from the superseded files' names and
%  was wrong), local system,
...
Figure~\ref{fig:solver-bench} compares the total stage time of all linear solver for all molecules.
...
% TODO (prose): write the cluster-comparison paragraph, mirroring the
% local one above and the energy-calculation local/cluster split in
% \ref{sec:evaluation-results-energy-calculation}. Figures~\ref{fig:solver-bench-cluster}
% and Table~\ref{tab:solver-bench-cluster} cover the same sweep A (LIS /
% AMGX default / AMGX tuned) run on the cluster: 6VYB and 1VSZ on a single
% A100 node at 16 ranks (test5/leafsweep_6VYB, test6/leafsweep_1VSZ), H1N1
% on an H200 node at 8 ranks (test7) with only LIS and AMGX (tuned) captured
% (no default-AMGX run exists for H1N1) -- called out automatically in
% \solverCaveatssolverbenchcluster.
% Numbers for reference: the LIS/AMGX(tuned) gap shrinks on the cluster --
% 6VYB goes from 14.88x locally to 8.47x, 1VSZ from 14.52x to 6.99x, versus
% H1N1's 12.30x. Table~\ref{tab:solver-bench-cluster}'s host-time column shows
% why: AMGX's own setup+solve time roughly halves on the A100 (the GPU itself
% is faster), but the host remainder is flat or slightly higher than locally
% despite going from 4 to 16 ranks -- the cluster's weaker per-core CPU offsets
% the extra parallelism there. LIS's stage time (all host, and the only sweep
% A configuration whose host time IS the whole distributed solve) still drops
% by ~2x on 16 ranks, which is what shrinks the gap.
\input{figures/solver_bench_cluster.tex}
\input{figures/solver_bench_cluster_table.tex}
