...
\caption[\texttt{create\_markers} speedup from bulk ray collection]{\textbf{\snakecase{create\_markers} vs.\ \snakecase{create\_markers\_fast} wall time, 6VYB (test1).} Median over 3 runs, whiskers spanning fastest to slowest; the label above each bar is that median in seconds, with the speedup relative to \snakecase{create\_markers} in parentheses. Measured on the local system (Table~\ref{tab:test-systems}).}
