...
%  Each group is broken out in its own detailed figure (fmm_g1 .. fmm_g8).
% ------------------------------------------------------------------------
...
\node[fmmgrp, fit=(f1)] (G6) {};

% =============== 7  L2L ===============
\coordinate (p7) at (f1.west |- G6.south);
\node[b, anchor=north west] (g1) at ([yshift=-9mm]p7) {L2L level sweep};
\node[fmmgrp, fit=(g1)] (G7) {};

% =============== 8  fused evaluation ===============
\coordinate (p8) at (0,0 |- G7.south);
\node[w,  anchor=north west] (h1) at ([yshift=-11mm]p8) {Fused L2P $+$ P2P leaf kernel};
\node[wc, anchor=north west] (h2) at ([yshift=-1.8mm]h1.south west) {Host reduction};
\node[fmmgrp, fit=(h1)(h2)] (G8) {};
...
\node[fmmlbl] at ([xshift=4mm]G6.north west) {\bfseries 6\ \ M2L};
\node[fmmlbl] at ([xshift=4mm]G7.north west) {\bfseries 7\ \ L2L};
\node[fmmlbl] at ([xshift=4mm]G8.north west) {\bfseries 8\ \ Fused evaluation};

% ---------- flow ----------
\foreach \x/\y in {a1/a2, a2/a3, a3/a4, c1/c2, c2/c3, h1/h2}
...
% top-left of the borders, so a centred arrow would strike through them.
\coordinate (x5) at ([xshift=-8mm]G5.north east);
\coordinate (x6) at ([xshift=-8mm]G6.north east);
\coordinate (x7) at ([xshift=-8mm]G7.north east);
\draw[fmmflow] (x6 |- G6.south) -- node[fmmedge, pos=0.5] {\texttt{d\_local} (partial)} (x7);
...
\draw[fmmflow] (x5 |- G5.south) -- node[fmmedge, pos=0.9] {near field} (x5 |- G8.north);
\draw[fmmflow] (x7 |- G7.south) -- node[fmmedge, pos=0.5] {far field} (x7 |- G8.north);
...
\caption[FMM implementation: pipeline overview]{%
    ...
    kernels in Figures~\ref{fig:fmm-g1}--\ref{fig:fmm-g8}.}
